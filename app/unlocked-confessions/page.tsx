...
const Page = async () => {
  ...
  return (
    <div className="min-h-screen bg-campus-cream py-10 px-4 flex flex-col">
      ...
      {unlockedConfessions.length > 0 ? (
          ...
          {unlockedConfessions.map((confession: any) => (
            ...
            >
              ...
              <div className="text-sm text-muted-foreground font-inter flex items-center justify-between">
                <div>
                  ❤️ {confession.likesCount} Likes &nbsp; 💬{" "}
                  {confession.commentsCount} Comments
                </div>
                <div className="text-right">
                  <span className="font-semibold text-campus-forest">
                    {confession.createdBy?.name}
                  </span>{" "}
        ...
        <div className="flex-grow flex justify-center mt-2 md:mt-8 text-center text-gray-500 text-lg font-semibold">
          You haven't Unlocked any Confessions Yet!
        </div>
      )}
    </div>
  );
